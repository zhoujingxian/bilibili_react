...
export default class Recommend extends React.Component {
    ...
    constructor(props) {
        super(props);
        pubsub.subscribe('up_loading', (msg, bl) => {
    ...
    componentWillUnmount() {
        pubsub.unsubscribe('up_loading');  //取消指定订阅
        this.setState = () => false;
    }
    ...
    async componentDidMount() {
        this.props.history.listen(async location => {
            if (location.pathname !== '/') {
                ...
                const d = await queryRec(location.pathname, this.state.channelList[location.pathname.split('/')[2]])
                this.setState({recData: d})
            }
        })
        console.log('rec')
        const data = await queryRec(this.props.location.pathname, this.state.channelList[this.props.match.params.tab])
        this.setState({recData: data})

    }

    render() {
        let data = this.state.channelList[this.props.match.params.tab]
        return (
